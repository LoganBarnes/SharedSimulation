...
// LTB Added throw options to definitions
#ifndef __THROW
#define __THROW
#endif

OPTIXU_INLINE float fminf(const float a, const float b) __THROW
{
  return a < b ? a : b;
}

OPTIXU_INLINE float fmaxf(const float a, const float b) __THROW
{
  return a > b ? a : b;
}

/** copy sign-bit from src value to dst value */
OPTIXU_INLINE float copysignf(const float dst, const float src) __THROW
{
  union {
    float f;
    unsigned int i;
